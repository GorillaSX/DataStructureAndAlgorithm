...
public:
    ...
    int shoppingOffers(vector<int>& price, vector<vector<int>>& special, vector<int>& needs) {
        int result = price * needs;

        for(auto offer : special)
        {
            if(needs >= offer)
            {
                vector<int> reminder = needs - offer;
                result = min(result, offer.back() + shoppingOffers(price, special, reminder));
            }
        }       
